...
if ($.isNode() && process.env.XL_URL4) {
    ...
    if (
        process.env.XL_URL4 &&
        process.env.XL_URL4.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleURL = process.env.XL_URL4.split(COOKIES_SPLIT);
    } else {
        middlexiaoleURL = process.env.XL_URL4.split();
    }
    if (
        process.env.XL_HEADER4 &&
        process.env.XL_HEADER4.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleHEADER = process.env.XL_HEADER4.split(COOKIES_SPLIT);
    } else {
        middlexiaoleHEADER = process.env.XL_HEADER4.split();
    }
}
