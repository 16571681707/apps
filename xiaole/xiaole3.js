...
多账号换行
第一步 微信扫码 https://raw.githubusercontent.com/ziye11/JavaScript/main/Task/.png  获取授权
第二步 
...
if ($.isNode() && process.env.XL_URL3) {
    ...
    if (
        process.env.XL_URL3 &&
        process.env.XL_URL3.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleURL = process.env.XL_URL3.split(COOKIES_SPLIT);
    } else {
        middlexiaoleURL = process.env.XL_URL3.split();
    }
    if (
        process.env.XL_HEADER3 &&
        process.env.XL_HEADER3.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleHEADER = process.env.XL_HEADER3.split(COOKIES_SPLIT);
    } else {
        middlexiaoleHEADER = process.env.XL_HEADER3.split();
    }
}
