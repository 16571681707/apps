...
if ($.isNode() && process.env.XL_URL2) {
    ...
    if (
        process.env.XL_URL2 &&
        process.env.XL_URL2.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleURL = process.env.XL_URL2.split(COOKIES_SPLIT);
    } else {
        middlexiaoleURL = process.env.XL_URL2.split();
    }
    if (
        process.env.XL_HEADER2 &&
        process.env.XL_HEADER2.indexOf(COOKIES_SPLIT) > -1
    ) {
        middlexiaoleHEADER = process.env.XL_HEADER2.split(COOKIES_SPLIT);
    } else {
        middlexiaoleHEADER = process.env.XL_HEADER2.split();
    }
}
